...
void Enemy::collisionAction() {  
   // Check if a bullet collides with any of the enemies  
   
	vector<Bullet>* buptr = (pGame->getPlayer().getBulletsPtr());
	for (int i = 0; i < pGame->getPlayer().getBulletCount(); i++) {  
       if (
		   this->collisionDetection
			(
				(*(pGame->getPlayer().getBulletsPtr()))[i]
			)
		   && buptr->size()
		   ) {
           // Remove the bullet at index i
			   buptr->erase((buptr)->begin());
		   i--; // Adjust the index after erasing an element
		   pGame->getPlayer().decreaseBulletCount(); //Decrease the number of bullets by one
       }
    }  
		buptr = nullptr;
}
...
vector<Enemy> EnemyManager::getAllEnemies() const {
