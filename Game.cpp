	...
	En(this)
{
	...
	{
		objects.push_back(&(En.getAllEnemies()[i]));
	}
	for (int i = 0; i < player.getBulletsPtr()->size(); i++)
	{
		objects.push_back(&(*(player.getBulletsPtr()))[i]);
	}
...
void Game::go()
{
	...
	{	
		...
		*/
		...
		for (int i = 0; i < En.getAllEnemies().size(); i++)
		{
			En.getAllEnemies()[i].collisionAction();
		}
		for (int i = 0; i < (*(player.getBulletsPtr())).size(); i++)
		{
			//(*(player.getBulletsPtr()))[i].collisionAction();
		}
		...
		for (int i = 0; i < objects.size(); i++)
		{
			...
			{
				if (objects[i]->collisionDetection(*objects[j])) {
					objects[i]->collisionAction();
				}
			}
		}
		Pause(20);
